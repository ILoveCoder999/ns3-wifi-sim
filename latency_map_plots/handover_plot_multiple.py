...
data_dir = Path(BASE_DIR) / "data/sim_res"
...
for fname in data_dir.iterdir():
    exp = "_".join(fname.name.split("_")[:2])
    if exp not in f_dict:
        f_dict[exp] = {"stalog":None, "assoc":None}
    if "stalog" in str(fname):
        f_dict[exp]["stalog"] = str(fname)
    else:
        f_dict[exp]["assoc"] = str(fname)

print(f_dict)

# %%
for exp, files in f_dict.items():
    if exp in {"db_2", "db_7", "db_9"}:
         continue

    print(files)

    with open(files["assoc"]) as f:
        assoc_log = json.load(f)

    with open(files["stalog"]) as f:
        sta_log = json.load(f)
    ...
    fig.savefig(plot_dir / "{}_latency_avg.{}".format(exp, IMG_FORMAT))
    ...
    plt.tight_layout()
    plot_int_aps(ax, x_pos_ap, x_pos_int)
    fig.savefig(plot_dir / "{}_latency_99perc.{}".format(exp, IMG_FORMAT))
    ...
    plt.tight_layout()
    plot_int_aps(ax, x_pos_ap, x_pos_int)
    fig.savefig(plot_dir / "{}_latency_99.9perc.{}".format(exp, IMG_FORMAT))
    ...
    plt.tight_layout()
    plot_int_aps(ax, x_pos_ap, x_pos_int)
    fig.savefig(plot_dir / "{}_attempts_avg.{}".format(exp,IMG_FORMAT))
    ...
    plt.tight_layout()
    plot_int_aps(ax, x_pos_ap, x_pos_int)
    fig.savefig(plot_dir / "{}_dropped_%.{}".format(exp, IMG_FORMAT))
    ...
    plt.tight_layout()
    plot_int_aps(ax, x_pos_ap, x_pos_int)
    fig.savefig(plot_dir / "{}_snr.{}".format(exp, IMG_FORMAT))
