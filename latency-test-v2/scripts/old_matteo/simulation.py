...
from utils.V2 import JsonConfig, JsonConfigEncoder
...
def run_simulation(args: SimulationArgs) -> Tuple[SimulationArgs, float]:
    ...
    _, err = subprocess.Popen([
        str(Path(__file__).parent.joinpath("../../build/latency-test-v2/ns3-dev-latency-test-v2-optimized").resolve()),  # TODO: da modificare
        json.dumps(args.parameters, separators=(',', ':'), cls=JsonConfigEncoder),
        f"{args.outputFilePath}.tmp"
...
def main(args: Args):
    ...
    if not path.exists(REPORT_FILE.format(args.out)):
        df = pd.DataFrame([
            {
                "Execution time": 0.0,
                "Configuration": json.dumps(experiment, cls=JsonConfigEncoder)
        ...
        for future in as_completed(futures):
            try:
                ...
                df.loc[ex_args.id] = {
                    ...
                    "Configuration": json.dumps(ex_args.parameters, cls=JsonConfigEncoder)
                }
                df.to_csv(REPORT_FILE.format(args.out), sep=" ", index_label="Id")
