...
from utils.handover import HandoverConfig
...
PROGRAM = "build/handover/ns3-dev-handover-optimized"

#CONF_FILE = "latency-test-v2/test_beacons_ap/test_beacons_ap.json"
#OUT_FOLDER = "latency-test-v2/test_beacons_ap"

CONF_FILE = "latency-test-v2/test_tii/tii_simulations_04.json"
OUT_FOLDER = "latency-test-v2/test_tii/sim_res"
...
def main():
    ...
    for idx, jconf in enumerate(json_configs):
        ...
        jconf = HandoverConfig(**jconf)
        sim_conf = json.dumps(asdict(jconf), separators=(',', ':'))
        staLogFile = Path(BASE) / OUT_FOLDER / "{}_{}_stalog.json".format(OUT_PREFIX, idx)
        assocLogFile = Path(BASE) / OUT_FOLDER / "{}_{}_assoc.json".format(OUT_PREFIX, idx)

        command = [
                    str(program),
                    '--jsonConfig={}'.format(sim_conf),
                    '--staLogFile={}'.format(str(staLogFile)),
                    '--assocLogFile={}'.format(str(assocLogFile)),
                    "--inlineConfig"
                ]
