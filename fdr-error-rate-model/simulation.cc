...
main(int argc, char* argv[])
{
    ...
    //Per evitare che faccia ARP nel mezzo, che attiverebbe la trasmissione a blocchi
    Config::SetDefault("ns3::ArpCache::AliveTimeout", TimeValue(Seconds(simulationTime)));
    ...
    std::cout << std::setw(5) << "index" << std::setw(6) << "MCS" << std::setw(13) << "Rate (Mb/s)"
              << std::setw(12) << "Tput (Mb/s)" << std::setw(6) << "Sent" << std::setw(10) << "Received"
              << std::setw(8) << "FDR  "
              << std::setw(12) << "Signal (dBm)" << std::setw(12) << "Noise (dBm)" << std::setw(9) << "SNR (dB)"
              << std::endl;
    for (uint16_t i = startIndex; i <= stopIndex; i++)
    {
        ...
        Ptr<ErrorModel> errorModel = CreateObject<MyPacketsErrorModel>();
        if (wifiType == "ns3::YansWifiPhy")
        {
            YansWifiChannelHelper channel;
            // channel.AddPropagationLoss("ns3::EmptyPropagationLossModel");
            channel.SetPropagationDelay("ns3::ConstantSpeedPropagationDelayModel");
            phy.SetErrorRateModel(errorModelType);
            //phy.Set("PostReceptionErrorModel", PointerValue(errorModel));
        ...
        else if (wifiType == "ns3::SpectrumWifiPhy")
        {
            ...
            // Ptr<EmptyPropagationLossModel> lossModel = CreateObject<EmptyPropagationLossModel>();
            ...
            spectrumPhy.Set("PostReceptionErrorModel", PointerValue(errorModel));
            //spectrumPhy.SetPreambleDetectionModel("ns3::MyPreambleDetectionModel");
        ...
        if (wifiType == "ns3::YansWifiPhy")
        {
            ...
            mac.SetType("ns3::ApWifiMac", "Ssid", SsidValue(ssid),
                    "BeaconInterval",
                    TimeValue(MicroSeconds(1024 * 200)));
        ...
        if (udp)
        {
            ...
            const auto packetInterval = 0.5;
            //Config::ConnectWithoutContext("/NodeList/*/ApplicationList/*/$ns3::UdpServer/Rx", MakeCallback(UdpServerRx));
        ...
        if (enablePcap)
        {
            ...
            ss << "wifi-spectrum-per-example-ap-" << i;
            phy.EnablePcap(ss.str(), apDevice);
            ss2 << "wifi-spectrum-per-example-sta-" << i;
            phy.EnablePcap(ss2.str(), staDevice);
        }
        g_signalDbmAvg = 0;
        g_noiseDbmAvg = 0;
        g_samples = 0;
        ...
        double measuredFdr = 0;
        if (udp)
        {
            // UDP
            totalPacketsSent = DynamicCast<UdpClient>(clientApp.Get(0))->GetTotalTx() / payloadSize;
        ...
        measuredFdr = (double)totalPacketsThrough / totalPacketsSent;
        std::cout << std::setw(5) << i << std::setw(6) << (i % 8) << std::setprecision(2)
                  << std::fixed << std::setw(10) << datarate << std::setw(12) << throughput
                  << std::setw(9) << totalPacketsSent
                  << std::setw(8) << totalPacketsThrough
                  << std::setprecision(2) << std::fixed << std::setw(8) << measuredFdr;
        if (totalPacketsThrough > 0)
        {
            std::cout << std::setw(12) << g_signalDbmAvg << std::setw(12) << g_noiseDbmAvg
                      << std::setw(8) << (g_signalDbmAvg - g_noiseDbmAvg) << std::endl;
        }
        else
        {
            std::cout << std::setw(12) << "N/A" << std::setw(12) << "N/A" << std::setw(8) << "N/A"
                      << std::endl;
        }
        Simulator::Destroy();
    }
