    ...
    while ((pkt = s->Recv ()))
      {
        ...
        // 终点服务器判断：只有当本服务器是 3D 路由路径的最后一跳时，才终止接收
        if (m_id == path.back())
          {
        ...
        // 中间服务器路由转发：基于当前 3D 节点在 path 中的位置，寻找下一跳轴向中转服务器
        uint16_t next_hop = 0;
        bool found = false;
...
int main (int argc, char *argv[])
{
  ...
  uint32_t uniformFlows = 200000; // 维持 20 万个高负载随机包设定
      ...
      for (uint32_t k = 0; k < uniformFlows; ++k)
        {
          ...
          Simulator::Schedule (Seconds (when), &MeshHost::Send, apps[s],
                               d, pktBytes, id++);
          when += 5e-7; // 20万个流量突发忙碌窗口约为0.1~0.2秒
        }
    }

  // 模拟剪裁时间窗：设置为 1.5 秒以精准包裹流量空腔
  ...
  // =================================================================
  // 【3D 集群能耗模型】：精准的“静态2000W / 满载8000W”分段能耗模型
  // =================================================================
  ...
  double E_dynamic_bit = 10e-12;   // 高速网卡收发动态线传能耗：10 pJ/bit

  // 1.5s 周期内集群持续消耗的基础静态能耗 (1331台服务器)
  double staticEnergy = N * P_static_node * simDuration;
  
  // 满载额外计算能耗（仅在真正通信忙碌的 rxDurationSec 时间窗内阶跃产生）
