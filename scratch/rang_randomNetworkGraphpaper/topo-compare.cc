 ...
 *
 * ═══════════════════════════════════════════════════════════════════════
 * 功能：一次性对比以下拓扑在同等条件下的性能
 *
 *  ┌──────────────────────────────────────────────────────────────────┐
 *  │  拓扑          节点  度/链路   特点                             │
 *  ├──────────────────────────────────────────────────────────────────┤
 *  │  RNG           100   d=16 RRG  准随机图，近似 Amazon RNG 论文   │
 *  │  Fat-Tree 3层  98    K=14      经典三层 Clos，工业基线          │
 *  │  Leaf-Spine 2层 100  K=20 nL=10 两层非阻塞胖树                 │
 *  │  Dragonfly     100   a=5,g=11  低直径，跨组全互联               │
 *  └──────────────────────────────────────────────────────────────────┘
 *
 * ═══════════════════════════════════════════════════════════════════════
 * 流量场景（对应 RNG 论文 §9.3 Figure 13）
 *   clique(f)  : f*N 节点两两全互联
 *   hubs(f)    : f*N 个 Hub，所有节点向 Hub 发包
 *   matching(f): f*N/2 对随机配对，双向发包
 *   uniform    : 随机均匀打流（基准）
 *
 * ═══════════════════════════════════════════════════════════════════════
 * 选项
 *   --topo       rng|fattree|leafspine|dragonfly|all  （默认 all）
 *   --pattern    clique|hubs|matching|uniform|all     （默认 all）
 *   --f          活跃比例 0~1                          （默认 0.3）
 *   --scanF      1=扫描多个 f 值（0.1~1.0）重现 Figure 13
 *   --linkRate   链路速率                              （默认 100Gbps）
 *   --linkDelay  链路延迟                              （默认 200ns）
 *   --queuePkts  队列深度（包数）                      （默认 8）
 *   --pktBytes   包大小（字节）                        （默认 1024）
 *
 * ═══════════════════════════════════════════════════════════════════════
 * 输出
 *   控制台：对比表格（sent/delivered/drop%/tput/p99 latency/energy）
 *   文件   ：topo_compare_results.csv
 */
...
//  全局统计（每次 RunSim 前清零）
// ═══════════════════════════════════════════════════════════════════════
struct Probe { double sentNs, recvNs; };
    ...
    uint32_t           burstPkts)
{
    ...
    // ── 拓扑参数 ──
    // RNG: n=100, d=16
    // FatTree 3层: K=14 → 98 ToR, 686 servers (仅模拟 ToR 层)
    // LeafSpine 2层: nLeaf=10, nSpine=20 (K=20), N=10*10=100 servers
    // Dragonfly: a=5, g=a*a+1=26... 实际用 a=4,g=17 → N=4*4*17=272 服务器，缩至 a=3,g=10 → N=90
    //            为对齐取 a=4, p=4, h=4, g=17 → N=4*4*17=272 → 太多
    //            用 a=3, p=3, h=3, g=10 → N=3*3*10=90 ≈ 100

    uint32_t N_ep = 0;       // 端点（服务器/ToR）数
    uint32_t N_sw = 0;       // 额外交换机数（fat-tree 用）
    double   P_sw = 300.0;   // 交换机功率(W)
    Ptr<UniformRandomVariable> rng = CreateObject<UniformRandomVariable>();

    NodeContainer eps;   // 端点节点
    NodeContainer sw1, sw2, sw3; // 各层交换机（视拓扑而定）
    ...
    if(topo=="rng"){
        ...
        // ─ Fat-Tree 3层 K=14：14 pods × 7 ToR = 98 ToR（端点） ─
        const uint32_t K=14, Npod=K, Ntor=K/2, Nagg=K/2, Ncore=(K/2)*(K/2);
        N_ep  = Npod*Ntor*Ntor; // 实际使用服务器数 = K^3/4 = 686，但此处仅模拟 ToR 层
        // 简化：以 ToR 节点为端点，不单独建服务器节点
        uint32_t N_tor_total = Npod*Ntor;  // = 98 ToR 节点
        N_ep = N_tor_total;
        N_sw = Npod*Nagg + Ncore;
        eps.Create(N_ep);  // ToR 节点
        sw1.Create(Npod*Nagg);  // Agg 节点
        sw2.Create(Ncore);      // Core 节点
        ...
        // ─ Leaf-Spine 2层：nLeaf=10, nSpine=10, 每 Leaf 10 台服务器 → N=100 ─
        const uint32_t nLeaf=10, nSpine=10, nSrv=10; // nSrv per leaf
        N_ep = nLeaf*nSrv;
        N_sw = nLeaf+nSpine;
        ...
        // ─ Dragonfly a=5, p=5, h=5, g=a*h+1=26, N=p*a*g=650（偏大）
        // 改用 a=3, p=3, h=3, g=10, N=3*3*10=90 ≈ 100 ─
        const uint32_t A=3,P=3,H=3,G=A*H+1;  // G=10, N=90
        N_ep=P*A*G;  // = 90
        uint32_t nRouters=A*G; // = 30 路由器
        ...
        // Inter-Group（全局链路：对每对 Group 连一条）
...
{
    // 同 pattern+f 组的 oversub 对比
    printf("\n  [%s f=%.2f]  过订阅比（越低越好）：",pat.c_str(),f);
    for(auto &r:results)
        if(r.pattern==pat && std::abs(r.f-f)<1e-6)
            printf("  %s=%.2f",r.topo.c_str(),r.oversub);
    printf("\n");
}
...
int main(int argc, char *argv[])
{
    ...
    std::cout<<"╔══════════════════════════════════════════════════════════════╗\n"
             ...
             <<"  Patterns   : "<<patternArg<<"\n"
             <<"  f values   : ";
    for(double v:fVals) std::cout<<v<<" "; std::cout<<"\n"
    ...
    for(auto &r:allResults)
        printf("  %-12s %-10s %4.2f  →  %.3f  (delivery %.1f%%  tput %.2f Gbps)\n",
               r.topo.c_str(),r.pattern.c_str(),r.f,
               r.oversub, r.delivRate*100, r.tputGbps);

    std::cout<<"\n  Wrote: topo_compare_results.csv\n";

    // 提示论文对应结论
    std::cout<<"\n  ── RNG 论文 Figure 13 预期结论 ──────────────────────────────\n"
             <<"  ✓ clique/hubs 大 f：RNG 过订阅比 < Fat-Tree（路径多样性优势）\n"
             <<"  ✓ matching  小 f：Fat-Tree 略优（短路集中，利用率高）\n"
             <<"  ✓ Leaf-Spine 在均匀流量下接近非阻塞（过订阅≈1）\n"
             <<"  ✓ Dragonfly 跨 Group 路径短，incast 表现好\n"
             <<"  ────────────────────────────────────────────────────────────\n\n";
    return 0;
}
